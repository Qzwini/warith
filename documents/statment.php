...
<body>
    ...
    <?php
    $resultsPerPage = 8;
    $numberOfResults = $database->prepare("SELECT * FROM `statement` ");
    ...
    $results = $database->prepare("SELECT * FROM `statement` order by Id desc LIMIT " . $resultsPerPage . " OFFSET " . ($page - 1) * $resultsPerPage);
    $results->execute();
            ...
            <div class="col-md-3 col-sm-6">
                <div class="card">
                    ...
                    <div class="card-body news">
                        <b class="card-title"><?php echo $result['number'] ?></b>
                        ...
                        <!-- <a href="<?php
                        //  echo $result['linki'] 
                        ?>" class="btn btn-light" target="_blank"> تحميل </a> -->
                    </div>
                </div>
            </div>
            <?php } ?>
            ...
            <div class="col-sm">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <p class="card-text"> <?php $description = substr($result['description'], 0, 150);
                                                    $desc = substr($description, 0, strrpos($description, ' ')) . '...';

                                                    echo $desc;

                                                    ?></p>
                        <?php echo ' <a href="statment-more.php?id=' . $result['ID'] . '  " class="btn btn-warning col-white"> قراءة المزيد </a>'; ?>
                    </div>
            ...
            <nav>
                <ul class="pagination justify-content-center ">
                    ...
                    for ($count = 1; $count <= $totalPages; ++$count) {
                        if ($page == $count) {
                            $count = $page;
                            echo ' <li class="page-item active"> <a class="page-link" href="statment.php?page=' . $count . '">' . $count . '</a>  </li>';
                        } else {
                            echo ' <li class="page-item"> <a class="page-link" href="statment.php?page=' . $count . '">' . $count . '</a>  </li>';
                        }
                    }
