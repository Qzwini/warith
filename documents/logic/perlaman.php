...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($tempname, $folder)) {
        ...
        if (!empty($filename)) {
            $sql = "INSERT INTO parliament (name,File_Name,description) VALUES ('$name','$filename','$description')";
    ...
    if (empty($filename)) {
        ...
        $sql = "Update parliament SET name= '$name',File_Name = '$imgs', description = '$description' where ID= " . $_POST['id'];
        mysqli_query($link, $sql);
        ...
        if (move_uploaded_file($tempname, $folder)) {
            // Get all the submitted data from the form 
            if (!empty($filename)) {
                $sql = "Update parliament SET name= '$name' ,File_Name = '$filename', description = '$description' where ID= " . $_POST['id'];;
                mysqli_query($link, $sql);
            }
            $msg = "parliament update successfully";
            header('Location: ../dashboard.php?active=parliament&message=' . $msg);
