...
<body>
    ...
    <?php
    $resultsPerPage = 10;
    $numberOfResults = $database->prepare("SELECT * FROM `parliament`");
    ...
    $results = $database->prepare("SELECT * FROM `parliament` order by Id desc LIMIT " . $resultsPerPage . " OFFSET " . ($page - 1) * $resultsPerPage);
    $results->execute();
    ...
    <div class="container mt-10">
        <div class="row ">
            ...
            <div class="col-md-3 col-sm-6">
                <div class="card">
                    <div class="imgparliament">
                        <img src="<?php echo '../uploads/' . $result['File_Name'] ?>" class="card-img-top imgparliament"
                            alt="...">
                    </div>
                    <div class="card-body news">
                        <b class="card-title"><?php echo $result['name'] ?></b>
                        <p class="card-text mt-2"> </p>
                        <a href="<?php echo $result['description'] ?>" class="btn btn-light" target="_blank"> تحميل </a>
                    </div>
                </div>
            </div>
            <?php } ?>
                ...
                <nav>
                    <ul class="pagination justify-content-center ">
                        ...
                        for ($count = 1; $count <= $totalPages; ++$count) {
                            if ($page == $count) {
                                $count = $page;
                                echo ' <li class="page-item active"> <a class="page-link" href="parliament.php?page=' . $count . '">' . $count . '</a>  </li>';
                            } else {
                                echo ' <li class="page-item"> <a class="page-link" href="parliament.php?page=' . $count . '">' . $count . '</a>  </li>';
                            }
                        }
