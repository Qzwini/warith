...
<body>
    ...
    <?php
    $resultsPerPage = 10;
    $numberOfResults = $database->prepare("SELECT * FROM `library`");
    ...
    $results = $database->prepare("SELECT * FROM `library` order by Id desc LIMIT " . $resultsPerPage . " OFFSET " . ($page - 1) * $resultsPerPage);
    $results->execute();
            ...
            <div class="col-md-3 col-sm-6">
                <div class="card">
                    <div class="imglibrary">
                        <div style="  background-color: #126e82 ;
">
                            <div style="padding:5px;color: #fff;">
                                <?php echo $result['type'] ?></div>
                            <img src="<?php echo '../uploads/' . $result['File_Name'] ?>"
                                class="card-img-top imglibrary" alt="...">
                        </div>

                    </div>
                    <div class="card-body library">
                        <b class="card-title"><?php echo $result['title'] ?></b>
                        ...
                        ?></p>
                        <a href="<?php echo $result['link'] ?>" class="btn btn-light" target="_blank"> تحميل </a>
                    </div>
                </div>
            </div>
            <?php } ?>
                ...
                <nav>
                    <ul class="pagination justify-content-center ">
                        ...
                        for ($count = 1; $count <= $totalPages; ++$count) {
                            if ($page == $count) {
                                $count = $page;
                                echo ' <li class="page-item active"> <a class="page-link" href="library.php?page=' . $count . '">' . $count . '</a>  </li>';
                            } else {
                                echo ' <li class="page-item"> <a class="page-link" href="library.php?page=' . $count . '">' . $count . '</a>  </li>';
                            }
                        }
