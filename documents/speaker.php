...
<body>
    ...
    <?php
    $resultsPerPage = 10;
    $numberOfResults = $database->prepare("SELECT * FROM `speaker`");
    ...
    $results = $database->prepare("SELECT * FROM `speaker` order by Id desc LIMIT " . $resultsPerPage . " OFFSET " . ($page - 1) * $resultsPerPage);
    $results->execute();
            ...
            <div class="col-md-3 col-sm-6">
                <div class="card">
                    <div class="imgspeaker">
                        <img src="<?php echo '../uploads/' . $result['File_Name'] ?>" class="card-img-top imgspeaker"
                            alt="...">
                    </div>
                ...
                <nav>
                    <ul class="pagination justify-content-center ">
                        ...
                        for ($count = 1; $count <= $totalPages; ++$count) {
                            if ($page == $count) {
                                $count = $page;
                                echo ' <li class="page-item active"> <a class="page-link" href="speaker.php?page=' . $count . '">' . $count . '</a>  </li>';
                            } else {
                                echo ' <li class="page-item"> <a class="page-link" href="speaker.php?page=' . $count . '">' . $count . '</a>  </li>';
                            }
                        }
